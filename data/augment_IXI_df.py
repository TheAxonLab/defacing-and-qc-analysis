...
# We use sort=False in the groupby to maintain the order in which the reports were rated
df = df.groupby(['randomized_id', 'rater_id'], sort=False).agg({'dataset': 'last', 'rating': 'last', 'time_sec': 'sum', 'artifacts': 'last', 'confidence': 'last', 'comments': 'last'}).reset_index()
